...
void NativeDatabaseBinding::registerNatives() {
  registerHybrid({
      ...
      makeNativeMethod("useForHttpSync",
                       NativeDatabaseBinding::useForHttpSync),
  });
}
...
int  NativeDatabaseBinding::useForHttpSync(){
    usedForHttpSync=true;
    facebook::react::NativeNPSyncModule::SetWorkingSqliteConnection(db);
    return 1;
}
int NativeDatabaseBinding::sqlite3_close() {
    ...
    if(usedForHttpSync){
        usedForHttpSync=false;
        facebook::react::NativeNPSyncModule::SetWorkingSqliteConnection(nullptr);
    }
    return ret;
}
} // namespace expo
